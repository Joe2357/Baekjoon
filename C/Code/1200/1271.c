...
typedef struct BigInteger {
	char digits[MAX_DIGITS + MORE_SPACE_FOR_INPUT]; // 각 자리 숫자 (역순 저장)
	int length;										// 숫자의 길이
...
// |a| >= |b| 이어야 함
BigInteger subAbs(BigInteger a, BigInteger b) {
	BigInteger result;
	result.length = 0;
	result.isNegative = false;
...
BigInteger modular(BigInteger a, BigInteger b) {
	...
	BigInteger temp = multiply(q, b);
	BigInteger r = subtract(a, temp);
	if (r.isNegative) { // 나머지는 양수로 보정
		r = add(r, b);
	}
	return r;
}
...
void printBigInteger(BigInteger a) {
	if (a.isNegative && (a.length == 1 && a.digits[0] == 0) == false) {
		printf("-");
	}
	for (int i = a.length - 1; i >= 0; --i) {
		printf("%d", a.digits[i]);
